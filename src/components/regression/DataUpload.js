...
const DataUpload = ({ onUpload, isLoading, error }) => {
  ...
  const handleFile = (file) => {
    ...
    if (file.size > 10 * 1024 * 1024) { // 10MB threshold
      const confirmMessage = `This file is ${formatFileSize(file.size)}, which is quite large. ` +
        `Large files may take longer to process and could potentially timeout. ` +
        `For best performance, consider using a smaller file.\n\n` +
          ...
          {isLoading && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-blue-700">Uploading and validating data...</span>
              ...
              <p className="text-xs text-blue-600">
                This may take up to 2 minutes for large files. The system will retry automatically if there are connection issues.
              </p>
            </div>
          )}
