...
const MemberDashboard = () => {
  ...
  const [expandedActionItems, setExpandedActionItems] = useState(new Set());
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Load guide progress for all guides
        const allGuideProgress = {};
        const guideIds = ['AIImplementationPlaybook', 'AIReadinessAssessment', 'AIUseCaseROIToolkit', 'AIStrategyStarterKit', 'AIEthicsGovernance'];
        ...
        setGuideProgress(allGuideProgress);
  ...
  const convertMarkdownToHTML = (markdown) => {
    ...
    html = html.replace(/\|(.+)\|/g, (match) => {
      // Check if this is a table by looking for table separators
      const lines = match.split('\n').filter(line => line.trim());
      if (lines.length > 1 && lines[1].includes('|--')) {
        // This is a table, process it
        let tableHtml = '<div class="overflow-x-auto mb-4"><table class="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg overflow-hidden">';
        let isHeader = true;
        ...
        lines.forEach((line, index) => {
          ...
          if (cells.length > 0) {
            if (isHeader) {
              ...
              cells.forEach(cell => {
                tableHtml += `<th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider border-b">${cell.trim()}</th>`;
              });
              ...
              cells.forEach((cell, cellIndex) => {
                ...
                // Special styling for priority levels
                let cellClass = 'px-4 py-3 text-sm text-gray-900';
                if (cellContent.toLowerCase() === 'high') {
                  cellClass += ' font-medium text-red-600';
                } else if (cellContent.toLowerCase() === 'medium') {
                  cellClass += ' font-medium text-yellow-600';
                } else if (cellContent.toLowerCase() === 'low') {
    ...
    html = html
      ...
      // Numbered lists
      .replace(/^\d+\. (.*)$/gim, '<li class="mb-2 text-gray-700 list-decimal ml-6">$1</li>')
      ...
      // Code blocks
      .replace(/```([^`]+)```/g, '<pre class="bg-gray-100 p-3 rounded-lg overflow-x-auto mb-4"><code class="text-sm text-gray-800">$1</code></pre>')
    ...
    // Wrap lists - handle both ordered and unordered
    html = html.replace(/(<li class="[^"]*list-disc[^"]*".*?<\/li>)+/gs, (match) => `<ul class="space-y-1 mb-4">${match}</ul>`);
    html = html.replace(/(<li class="[^"]*list-decimal[^"]*".*?<\/li>)+/gs, (match) => `<ol class="space-y-1 mb-4">${match}</ol>`);
  ...
  const toggleActionItemExpansion = (actionItemId) => {
    const newExpanded = new Set(expandedActionItems);
    if (newExpanded.has(actionItemId)) {
      newExpanded.delete(actionItemId);
    } else {
      newExpanded.add(actionItemId);
    }
      ...
      {actionItems.length > 0 ? (
        <div className="space-y-6">
          {['learning', 'organizational', 'technical', 'process'].map(category => {
            ...
            return (
              <div key={category} className="bg-white rounded-lg shadow-sm p-6" style={{ borderTop: `4px solid ${colors.chestnut}` }}>
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold capitalize" style={{ color: colors.charcoal, fontFamily: 'Playfair Display' }}>
                    {category} Action Items
                  </h3>
                  <span className="text-sm" style={{ color: colors.khaki, fontWeight: '500' }}>
                    ...
                    return (
                      <div 
                        ...
                        className={`border rounded-lg transition-all duration-300 overflow-hidden ${
                          isCompleted ? 'bg-gray-50 border-gray-200' : 'bg-white border-gray-300 hover:shadow-md hover:border-chestnut/30'
                        }`}
                      >
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-start space-x-3 flex-1">
                              <input
                                ...
                                className="mt-1 w-4 h-4 rounded border-2 focus:ring-2"
                                style={{ 
                                  accentColor: colors.chestnut,
                                  borderColor: isCompleted ? colors.chestnut : colors.khaki 
                                }}
                              />
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-2">
                                  <h4 className={`font-semibold text-lg leading-tight ${
                                    isCompleted ? 'line-through text-gray-500' : 'text-charcoal'
                                  ...
                                  <div className="flex items-center space-x-2 ml-4">
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                      ...
                                      item.priority === 'medium' ? 'text-white' :
                                      'text-white'
                                    }`} style={{
                                      backgroundColor: item.priority === 'high' ? colors.chestnut :
                                                     item.priority === 'medium' ? colors.khaki :
                                                     colors.pearl
                                    }}>
                                      {item.priority}
                                    </span>
                                ...
                                <p className="text-sm text-gray-600 leading-relaxed" style={{ fontFamily: 'Lato' }}>
                                ...
                                <div className="flex items-center space-x-4 mt-3">
                                  {item.estimatedHours && (
                                    <span className="flex items-center text-xs" style={{ color: colors.khaki }}>
                                      <Clock className="w-3 h-3 mr-1" />
                                      {item.estimatedHours}h
                                    </span>
                                  )}
                                  {item.generatedBy === 'crewai_agent' && (
                                    <span className="flex items-center text-xs" style={{ color: colors.chestnut }}>
                        ...
                        <div className={`transition-all duration-300 overflow-hidden ${
                          ...
                          <div className="px-4 pb-4" style={{ borderTop: `1px solid ${colors.pearl}` }}>
                            <div className="pt-4 space-y-4">
                              
                              {/* Full Description/Content */}
                              {item.description && (
                                <div className="prose prose-sm max-w-none">
                                  <h5 className="font-semibold mb-2" style={{ color: colors.charcoal, fontFamily: 'Lato' }}>
                                    Full Details
                                  </h5>
                                  {typeof item.description === 'string' ? (
                                    <div 
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: convertMarkdownToHTML(item.description)
                                      }}
                                    />
                                  ) : typeof item.description === 'object' && item.description.title ? (
                                    <div className="text-gray-700" style={{ fontFamily: 'Lato' }}>
                                      <p className="font-medium mb-2">{item.description.title}</p>
                                      {item.description.cost && <p className="text-sm mb-1"><strong>Cost:</strong> {item.description.cost}</p>}
                                      {item.description.duration && <p className="text-sm mb-1"><strong>Duration:</strong> {item.description.duration}</p>}
                                      {item.description.details && <p className="text-sm">{item.description.details}</p>}
                                    </div>
                                  ) : (
                                    <div 
                              ...
                              {/* CrewAI Intelligence Section */}
                              {item.generatedBy === 'crewai_agent' && (
                                <div className="rounded-lg p-4" style={{ backgroundColor: `${colors.bone}` }}>
                                  <div className="flex items-center mb-3">
                                    <Brain className="w-5 h-5 mr-2" style={{ color: colors.chestnut }} />
                                    <h6 className="font-semibold" style={{ color: colors.charcoal, fontFamily: 'Lato' }}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                      {item.metadata.phase && (
                                        <div className="flex justify-between">
                                          <span className="font-medium" style={{ color: colors.charcoal }}>Implementation Phase:</span>
                                          <span style={{ color: colors.chestnut }}>{item.metadata.phase}</span>
                                        </div>
                                      )}
                                      {item.metadata.timeline && (
                                        <div className="flex justify-between">
                                          <span className="font-medium" style={{ color: colors.charcoal }}>Expected Timeline:</span>
                                          <span style={{ color: colors.khaki }}>{item.metadata.timeline}</span>
                                        </div>
                                      )}
                                      {item.metadata.impact && (
                                        <div className="flex justify-between">
                                          <span className="font-medium" style={{ color: colors.charcoal }}>Expected Impact:</span>
                                          <span style={{ color: colors.chestnut }}>{item.metadata.impact}</span>
                                        </div>
                                      )}
                                      {item.metadata.complexity && (
                                        <div className="flex justify-between">
                                          <span className="font-medium" style={{ color: colors.charcoal }}>Complexity Level:</span>
                                          <span style={{ color: colors.khaki }}>{item.metadata.complexity}</span>
                                        </div>
                                      )}
                                    </div>
                                  )}
                                  ...
                                  {item.tags && item.tags.includes('strategic-initiative') && (
                                    <div className="mt-3">
                                      <span className="px-3 py-1 text-xs font-medium rounded-full text-white" style={{ backgroundColor: colors.chestnut }}>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                {item.source && (
                                  <div className="flex items-center justify-between p-2 rounded" style={{ backgroundColor: colors.pearl }}>
                                    <span className="font-medium" style={{ color: colors.charcoal }}>Source:</span>
                                    <span className="capitalize" style={{ color: colors.khaki }}>
                                ...
                                {item.generatedBy && item.generatedBy !== 'crewai_agent' && (
                                  <div className="flex items-center justify-between p-2 rounded" style={{ backgroundColor: colors.pearl }}>
                                    <span className="font-medium" style={{ color: colors.charcoal }}>Generated By:</span>
                                    <span className="capitalize" style={{ color: colors.khaki }}>
                              ...
                              {item.tags && item.tags.length > 0 && !item.tags.includes('strategic-initiative') && (
                                <div>
                                  <h6 className="font-medium mb-2 text-sm" style={{ color: colors.charcoal, fontFamily: 'Lato' }}>Tags:</h6>
                                  <div className="flex flex-wrap gap-2">
                                    {item.tags.map((tag, index) => (
                                      <span 
                                        key={index} 
                                        className="px-2 py-1 text-xs rounded-full"
                                        style={{ backgroundColor: colors.pearl, color: colors.charcoal }}
                                      >
                                        {tag.replace(/-/g, ' ')}
                                      </span>
                                    ))}
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-lg mb-2" style={{ fontFamily: 'Playfair Display', fontWeight: 'bold', color: colors.charcoal }}>No action items yet</p>
            <p className="text-sm" style={{ fontFamily: 'Lato' }}>Complete assessments to get personalized recommendations that will help your organization succeed with AI.</p>
